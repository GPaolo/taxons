...
class ConvAutoEncoder(nn.Module):
  ...
  def __init__(self, device=None, learning_rate=0.001, **kwargs):
    ...
    self.encoder_ff = nn.Sequential(nn.Linear(484, kwargs['encoding_shape'], bias=False), nn.LeakyReLU()).to(self.device)
    ...
    #                              nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=5, stride=2, bias=False), nn.LeakyReLU(), # 7 -> 17
    #                              nn.ConvTranspose2d(in_channels=16, out_channels=16, kernel_size=7, stride=1, bias=False), nn.LeakyReLU(), # 17 -> 23
    #                              nn.ConvTranspose2d(in_channels=16, out_channels=8, kernel_size=5, stride=3, bias=False), nn.LeakyReLU(), # 23 -> 71
    #                              nn.ConvTranspose2d(in_channels=8, out_channels=3, kernel_size=5, bias=False), nn.LeakyReLU()).to(self.device) # 71 -> 75
    ...
    self.att = torch.softmax(torch.zeros(1, kwargs['encoding_shape']), dim=1)
  ...
  def training_step(self, x, rew):
    ...
    att = torch.softmax((feat - feat_avg) * r, dim=1)
